...
class App extends React.Component {
  ...
  addPodcast = async () => {
    ...
    if (url && valid === true) {
      const data = { url: url }
      try {
        await axios.post("http://localhost:3000/podcast/add", data);
      }
      catch (error) {
  ...
  deletePodcast = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/podcast/delete/${id}`);
    }
    catch (error) {
  ...
  getAllPodcasts = async () => {
    try {
      const podcasts = await axios.get('http://localhost:3000/podcast/all');
